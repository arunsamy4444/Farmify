...
useEffect(() => {
  toast.warning(
   ...
   const staticProducts = [
    ...
    // 👇 Optional: still fetch orders (works on both Render and local)
    axios
  ...
  const removeProduct = (productId) => {
    setSelectedOrders(
      selectedOrders.filter((item) => item.product._id !== productId)
    );
  };
  ...
  const placeOrder = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        navigate(
          `/payment/${userId}/${response.data.order._id}?total=${totalOrderPrice}`
        );
      })
      .catch((error) => {
  ...
  return (
    <div className="dashboard">
      ...
      <h1 className="text-4xl font-bold text-gray-900 text-center mt-6 mb-8">
        Buyer Dashboard
      </h1>
      ...
      {selectedOrders.length > 0 && (
        <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200 max-w-lg mx-auto">
          ...
          <div className="space-y-3">
            {["taluk", "district", "pincode", "villageTown"].map((field) => (
              <input
                key={field}
                type="text"
